...
void handleMenuScrollActions() {
  switch (menuMode) {
    ...
    case MENU_MODE_TIMER_MINUTES: {
      if (rotaryButtonUp) { programmDurationSeconds += 60; if (programmDurationSeconds > 3600) programmDurationSeconds = 3600; } 
    ...
    case MENU_MODE_TIMER_SECONDS: {
      if (rotaryButtonUp) { programmDurationSeconds += 1; if (programmDurationSeconds > 3600) programmDurationSeconds = 3600; } 
...
void screenAbout() {
  ...
  drawBottomString("1.2/" + String(finishedCount));
}
...
void screenSettingsMenu() {
  switch(menuSettingsPosition) {
    case MENU_SETTINGS_TANK: {
      drawIcon(ICON_TANK);
      drawBottomString(menuSettings[menuSettingsPosition] + (settingsTank == 0 ? "J1500" : (settingsTank == 1 ? "J2500" : "P4")));
      break;
    }
